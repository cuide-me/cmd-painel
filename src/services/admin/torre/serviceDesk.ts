...
/**
 * Gera ticket a partir de feedback tipo "Problema" ou "Reclamação"
 */
async function autoGenerateComplaintTicket(
  feedback: any,
  db: FirebaseFirestore.Firestore
): Promise<void> {
  try {
    const isComplaint = feedback.tipo === 'Reclamação' || feedback.tipo === 'Problema';
    
    const ticketData = {
      userId: feedback.userId || 'anônimo',
      userName: feedback.userName || feedback.nome || 'Usuário',
      userType: feedback.userType || feedback.perfil || 'family',
      source: 'complaint' as TicketSource,
      priority: (feedback.tipo === 'Problema' ? 'urgent' : 'high') as TicketPriority,
      status: 'open' as TicketStatus,
    ...
    await db.collection('tickets').add(ticketData);
    console.log(`[ServiceDesk] Ticket criado para ${feedback.tipo}: ${feedback.userId}`);
  } catch (error) {
    console.error('[ServiceDesk] Erro ao criar ticket de complaint:', error);
  }
}

/**
 * Gera ticket de oportunidade a partir de feedback tipo "Sugestão"
 */
async function autoGenerateOpportunityTicket(
  feedback: any,
  db: FirebaseFirestore.Firestore
): Promise<void> {
  try {
    const ticketData = {
      ...
      userName: feedback.userName || feedback.nome || 'Usuário',
      userType: feedback.userType || feedback.perfil || 'family',
      source: 'feature_request' as TicketSource,
    ...
    await db.collection('tickets').add(ticketData);
    console.log(`[ServiceDesk] Ticket de oportunidade criado: ${feedback.userId}`);
  } catch (error) {
    console.error('[ServiceDesk] Erro ao criar ticket de oportunidade:', error);
  }
}
...
export async function getServiceDeskSummary(): Promise<ServiceDeskSummary> {
  ...
  try {
    ...
    // 2. Buscar feedbacks tipo "Problema" e "Reclamação" e criar tickets
    const complaintsSnap = await db
      .collection('feedbacks')
      .where('tipo', 'in', ['Problema', 'Reclamação'])
      .where('createdAt', '>=', thirtyDaysAgo)
      .get();
    ...
    for (const doc of complaintsSnap.docs) {
      ...
      if (existingTicket.empty) {
        await autoGenerateComplaintTicket(feedback, db);
      }
    }

    // 3. Buscar feedbacks tipo "Sugestão" e criar tickets de oportunidade
    ...
    for (const doc of suggestionsSnap.docs) {
      ...
      if (existingTicket.empty) {
        await autoGenerateOpportunityTicket(feedback, db);
      }
    }

    // 4. Buscar todos os tickets ativos
