...
export async function getChurnMetrics(
  ...
  // Buscar assinaturas ativas no início do período
  ...
  const churnedSubs = await db.collection('subscriptions')
  ...
  // Customer Churn Rate
  const customerChurnRate = initialSubs.size > 0
    ? (churnedSubs.size / initialSubs.size) * 100 / months
    : 0;
  ...
  for (const doc of churnedSubs.docs) {
    const data = doc.data();
