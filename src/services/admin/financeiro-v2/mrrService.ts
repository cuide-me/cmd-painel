...
export async function getMRRMetrics(
  ...
  // Buscar assinaturas ativas (SIMPLIFICADO: apenas 1 WHERE)
  const subscriptionsRef = db.collection('subscriptions');
  
  // Current MRR - buscar todas ativas e filtrar no código
  ...
  currentSubs.forEach((doc: any) => {
    ...
    // Current MRR (assinaturas que expiram DEPOIS do mês atual)
    if (currentPeriodEnd > currentMonth) {
      currentMRR += amount;
    ...
    // Previous MRR (assinaturas que expiraram no mês anterior)
    if (currentPeriodEnd > previousMonth && currentPeriodEnd <= currentMonth) {
      previousMRR += amount;
